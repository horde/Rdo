 ...
 * @package  Rdo
 */
class DefaultList implements ArrayAccess, Iterator, Countable, RampageList
{
    /**
     * Rdo Query
     * @var mixed
     ...
     * requested. Can be a Query object, a literal SQL
     * query, or a tuple containing an SQL string and an array of bind
     * parameters to use.
     * @param Mapper $mapper Mapper to create objects for this list from.
     */
    public function __construct($query, $mapper = null)
    {
        if ($query instanceof Query) {
            ...
            if (!$mapper) {
                if (!$query->mapper) {
                    throw new RdoException('Mapper must be set on the Query object or explicitly passed.');
                }
                $mapper = $query->mapper;
            }
        ...
        if (!$mapper) {
            throw new RdoException('Mapper must be provided either explicitly or in a Query object');
        }
     ...
     * This method is executed when adding an item to the List
     ...
     * This method is executed when calling unset on a List index
     * @param Base $item  The item to add to the list.
