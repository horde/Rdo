...
class BaseQuery implements Query
{
     ...
     * @param Mapper $mapper The Mapper object governing this query.
     *
     * @return Query The full Query object.
     */
     ...
     * @todo Make $mapper non-optional in H6
     *
     * @param  Mapper  $mapper  Rdo mapper base class
     */
     ...
     * @param Mapper $mapper Rdo mapper base class
     *
     * @return Query Return the query object for fluent chaining.
     */
    public function setMapper($mapper)
    {
        ...
        foreach ($mapper->relationships as $relationship => $rel) {
            if (isset($rel['mapper'])) {
                ...
                if (is_null($m)) {
                    throw new RdoException('Unable to find a Mapper class for eager-loading relationship ' . $relationship);
                }
            }
     ...
     *
     * @return Query Returns self for fluent method chaining.
     */
    ...
    public function addRelationship($relationship, $args)
    {
        if (!isset($args['mapper'])) {
            throw new InvalidArgumentRdoException('Relationships must contain a Mapper object.');
        }
        if (!isset($args['table'])) {
        ...
        if (!isset($args['type'])) {
            $args['type'] = Constants::MANY_TO_MANY;
        }
        if (!isset($args['join_type'])) {
            switch ($args['type']) {
            case Constants::ONE_TO_ONE:
     ...
     *
     * @param Mapper $m1 Left-hand mapper
     * @param Mapper $m2 Right-hand mapper
    ...
    protected function _fillJoinPlaceholders($m1, $m2, $query)
    {
        ...
        foreach (array_keys($query) as $field) {
            ...
            if (preg_match('/^@(.*)@$/', $value, $matches)) {
                $q[$m1->tableAlias . '.' . $field] = new BaseQuery_Literal($m2->table . '.' . $matches[1]);
            } else {
